...
class Client {
  constructor (token) {
    ...
    this.session = null // string or null
  }

  async submitRpc (namespace, method, ...params) {
    const session = window.sessionStorage ? window.sessionStorage.getItem(SESSION_KEY) || this.session : this.session
    ...
    if (window && window.fetch) {
      ...
      if (window.sessionStorage) {
        window.sessionStorage.setItem(SESSION_KEY, this.session || '')
      }
